...
```{r}
if (model == "SPM"){
  estimate <- SPM_training_Normal(X = training$X, Y = training$Y, Ts = Ts, b = b, alpha = alpha, alpha_p = alpha_p, beta_p = beta_p, VI = TRUE)
  ...
  save.image(image_name)
  membership <-  BPM_membership_Bernoulli(X = testing$X, a = estimate[["a"]], rho = estimate[["rho"]], P = estimate[["P"]], VI = FALSE)
  save.image(image_name)
