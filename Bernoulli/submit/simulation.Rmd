...
```{r}
if (model == "SPM"){
  estimate <- SPM_training_Bernoulli(X = training$X, Y = training$Y, Ts = Ts, b = b, alpha = rep(1, k0 + k1), alpha_p = alpha_p, beta_p = beta_p, VI = TRUE)
  ...
  membership <- SPM_membership_Bernoulli(X = testing$X, Y = prediction$labels, a = estimate$a, rho = estimate$rho, Ts = Ts, P = estimate$P, VI = TRUE )
  
  save.image(image_name)
}else{
  estimate <- BPM_training_Bernoulli(X = training$X, b = b, alpha = rep(1, ntopic), alpha_p = alpha_p, beta_p = beta_p, ntopic = ntopic, VI = FALSE)
  save.image(image_name)
  membership <-  BPM_membership_Bernoulli(X = testing$X, a = estimate[["a"]], rho = estimate[["rho"]], P = estimate[["P"]], VI = TRUE)
  save.image(image_name)
  
  svm_BPM <- svm(factor(training[["Y"]]) ~ estimate[["U"]])
  prediction <- predict(svm_BPM, membership[["U"]]) == factor(testing[["Y"]])
  accuracy <- sum(prediction == factor(testing[["Y"]])) / Ntest
  print(accuracy)
